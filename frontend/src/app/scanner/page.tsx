...
type FrontPipeline={fullUniverse:StageDiag;activeSpot:StageDiag;allowedQuote:StageDiag;stableBase:StageDiag;uniqueBase:StageDiag;scanPool:StageDiag;liquidity:StageDiag;top30:StageDiag};
...
export default function ScannerPage(){
 const[timeframe,setTimeframe]=useState("15m");const[logs,setLogs]=useState<ScannerRun[]>([]);const[scanning,setScanning]=useState(false);const[progress,setProgress]=useState("Idle");const[error,setError]=useState<string|null>(null);const[autoRun,setAutoRun]=useState(false);const[countdown,setCountdown]=useState(AUTO_SCAN_SECONDS);const[frontPipeline,setFrontPipeline]=useState<FrontPipeline|null>(null);const[selectedStage,setSelectedStage]=useState("Scan Pool");const runningRef=useRef(false);const latest=logs[0];
 ...
 const runScanner=useCallback(async()=>{if(runningRef.current)return;runningRef.current=true;setScanning(true);setError(null);setProgress("Building max-200 scanner universe");try{
  const[er,tr]=await Promise.all([fetch(`${MARKET_API}/api/v3/exchangeInfo`,{cache:"no-store"}),fetch(`${MARKET_API}/api/v3/ticker/24hr`,{cache:"no-store"})]);if(!er.ok||!tr.ok)throw new Error("Unable to load Binance scanner universe");
  const exchange=await er.json() as ExchangeInfo;const tickers=await tr.json() as Ticker24h[];const tickerMap=new Map(tickers.map(t=>[t.symbol,Number(t.quoteVolume)]));
  const fullSymbols=exchange.symbols.map(s=>s.symbol);const activeRows=exchange.symbols.filter(s=>s.status==="TRADING"&&s.isSpotTradingAllowed!==false);const activeSymbols=activeRows.map(s=>s.symbol);const allowedRows=activeRows.filter(s=>ALLOWED_QUOTES.has(s.quoteAsset));const allowedSymbols=allowedRows.map(s=>s.symbol);const nonStableRows=allowedRows.filter(s=>!STABLE_BASES.has(s.baseAsset));const nonStableSymbols=nonStableRows.map(s=>s.symbol);
  const bestByBase=new Map<string,{symbol:string;baseAsset:string;quoteVolume:number}>();for(const s of nonStableRows){const q=tickerMap.get(s.symbol)??0;const current=bestByBase.get(s.baseAsset);if(!current||q>current.quoteVolume)bestByBase.set(s.baseAsset,{symbol:s.symbol,baseAsset:s.baseAsset,quoteVolume:q})}
  const uniqueRows=[...bestByBase.values()].sort((a,b)=>b.quoteVolume-a.quoteVolume);const uniqueSymbols=uniqueRows.map(x=>x.symbol);const scanPoolRows=uniqueRows.slice(0,SCAN_POOL_LIMIT);const scanPoolSymbols=scanPoolRows.map(x=>x.symbol);const liquidRows=scanPoolRows.filter(x=>x.quoteVolume>=MIN_QUOTE_VOLUME);const liquidSymbols=liquidRows.map(x=>x.symbol);const topRows=liquidRows.slice(0,SCAN_LIMIT);const topSymbols=topRows.map(x=>x.symbol);
  setFrontPipeline({fullUniverse:{passed:fullSymbols.length,dropped:0,dropped_symbols:[],passed_symbols:fullSymbols},activeSpot:diag(fullSymbols,activeSymbols),allowedQuote:diag(activeSymbols,allowedSymbols),stableBase:diag(allowedSymbols,nonStableSymbols),uniqueBase:diag(nonStableSymbols,uniqueSymbols),scanPool:diag(uniqueSymbols,scanPoolSymbols),liquidity:diag(scanPoolSymbols,liquidSymbols),top30:diag(liquidSymbols,topSymbols)});
  const markets:{symbol:string;quote_volume:number;candles:CandlePayload[]}[]=[];for(let i=0;i<topRows.length;i+=5){const batch=topRows.slice(i,i+5);setProgress(`Loading candles for Indicator Engine ${Math.min(i+batch.length,topRows.length)}/${topRows.length}`);const results=await Promise.all(batch.map(async x=>{try{return{symbol:x.symbol,quote_volume:x.quoteVolume,candles:await fetchCandles(x.symbol,timeframe)}}catch{return{symbol:x.symbol,quote_volume:x.quoteVolume,candles:[] as CandlePayload[]}}}));markets.push(...results)}
  setProgress("Indicator Engine → Scanner filters");const r=await fetch(`/api/scanner/run?timeframe=${timeframe}&min_quote_volume=${MIN_QUOTE_VOLUME}`,{method:"POST",cache:"no-store",headers:{"content-type":"application/json"},body:JSON.stringify(markets)});if(!r.ok){const b=await r.json().catch(()=>({}));throw new Error(b.detail??"Scanner Engine failed")};await loadLogs();setProgress("Scan complete");setCountdown(AUTO_SCAN_SECONDS)
 }catch(e){setError(e instanceof Error?e.message:"Scanner Engine failed");setProgress("Scan failed")}finally{setScanning(false);runningRef.current=false}},[timeframe,loadLogs]);
 ...
 const stages=useMemo(()=>{const p=latest?.pipeline;const topSymbols=frontPipeline?.top30.passed_symbols??[];
  const indicatorPassed=p?.indicator_analysis?.passed_symbols??[];const indicatorDiag:StageDiag|null=p?.indicator_analysis?diag(topSymbols,indicatorPassed):null;
  const volumePassed=p?.volume?.passed_symbols??[];const volumeDiag:StageDiag|null=p?.volume&&indicatorDiag?diag(indicatorPassed,volumePassed):null;
  const directionPassed=p?.direction?.passed_symbols??[];const directionDiag:StageDiag|null=p?.direction&&volumeDiag?diag(volumePassed,directionPassed):null;
  const shortlistPassed=p?.shortlist?.passed_symbols??latest?.candidates.map(c=>c.symbol)??[];const shortlistDiag:StageDiag|null=p?.shortlist&&directionDiag?diag(directionPassed,shortlistPassed):null;
  return[
   {title:"Exchange Metadata",rule:"Binance symbol metadata only · not analyzed",diag:frontPipeline?.fullUniverse??null},
   ...
   {title:"Scan Pool",rule:"Top 200 unique assets · LONG + SHORT combined",diag:frontPipeline?.scanPool??null},
   {title:"Liquidity",rule:"24h quote volume ≥ $10M",diag:frontPipeline?.liquidity??null},
   {title:"Top 30",rule:"Top 30 liquid assets for indicators",diag:frontPipeline?.top30??null},
   {title:"Indicator Analysis",rule:"Closed candles · EMA/RSI/MACD/volume",diag:indicatorDiag},
   {title:"Volume Filter",rule:"Latest closed volume ≥ 1.5x previous 20 avg",diag:volumeDiag},
   {title:"Direction Filter",rule:"RSI + MACD + LONG/SHORT alignment",diag:directionDiag},
   {title:"Final Shortlist",rule:"Eligible scored LONG/SHORT candidates",diag:shortlistDiag},
 ...
 return <div className="pageWrap">
  <div className="pageHeader"><div><p className="eyebrow">Market Intelligence</p><h1>Scanner</h1><p className="muted">Binance metadata → max 200 unique coin scan pool → top 30 → Indicator Engine → Volume → Direction → Final shortlist.</p></div><span className="modePill"><span />{scanning?"Scanner running":autoRun?"Auto scan active":"Scanner ready"}</span></div>
  <div className="statGrid"><div className="statCard"><span>Last Scan</span><strong>{latest?new Date(latest.timestamp).toLocaleTimeString():"—"}</strong><small className="neutral">{latest?.timeframe??timeframe} timeframe</small></div><div className="statCard"><span>Scan Pool</span><strong>{frontPipeline?.scanPool.passed??0}</strong><small className="neutral">max 200 unique assets total</small></div><div className="statCard"><span>LONG</span><strong className="positive">{latest?.long_candidates??0}</strong><small className="neutral">bullish opportunities</small></div><div className="statCard"><span>SHORT</span><strong className="negative">{latest?.short_candidates??0}</strong><small className="neutral">bearish opportunities</small></div></div>
  <section className="panel" style={{padding:18,marginBottom:14}}><div className="panelHead" style={{alignItems:"flex-start"}}><div><p className="eyebrow">Scanner Pipeline</p><h2>200-Coin Funnel Trace</h2><p className="muted" style={{marginTop:6}}>{progress}</p></div><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}><select value={timeframe} onChange={e=>setTimeframe(e.target.value)} disabled={scanning} style={controlStyle}>{TIMEFRAMES.map(tf=><option key={tf}>{tf}</option>)}</select><button onClick={()=>void runScanner()} disabled={scanning} style={runStyle}>{scanning?"Scanning…":"Run Now"}</button><button onClick={()=>{setAutoRun(v=>!v);setCountdown(AUTO_SCAN_SECONDS)}} style={autoRun?autoOnStyle:controlStyle}>{autoRun?"Auto Run ON":"Auto Run OFF"}</button><span className="periodTag">Next scan {autoRun?`${countdown}s`:"—"}</span></div></div>
...
const pipelineWrap={display:"grid",gridTemplateColumns:"repeat(12,minmax(125px,1fr))",gap:10,marginTop:18,overflowX:"auto",paddingBottom:4} as const;
