...
type PipelineDiag={backend_input?:BackendStageDiag;dedupe_liquidity?:BackendStageDiag;candles?:BackendStageDiag;volume?:BackendStageDiag;direction?:BackendStageDiag;shortlist?:BackendStageDiag};
...
export default function ScannerPage(){
 ...
 const runScanner=useCallback(async()=>{
  ...
  try{
   ...
   for(const s of nonStableRows){
    const q=tickerMap.get(s.symbol)??0;
    const current=bestByBase.get(s.baseAsset);
    if(!current||q>current.quoteVolume)bestByBase.set(s.baseAsset,{symbol:s.symbol,baseAsset:s.baseAsset,quoteVolume:q});
   }
   ...
   setFrontPipeline({
    ...
    activeSpot:diag(fullSymbols,activeSymbols),
    allowedQuote:diag(activeSymbols,allowedSymbols),
    stableBase:diag(allowedSymbols,nonStableSymbols),
    uniqueBase:diag(nonStableSymbols,uniqueSymbols),
    liquidity:diag(uniqueSymbols,liquidSymbols),
   ...
   for(let i=0;i<topRows.length;i+=5){
    ...
    setProgress(`Indicator data ${Math.min(i+batch.length,topRows.length)}/${topRows.length}`);
    const results=await Promise.all(batch.map(async x=>{try{return{symbol:x.symbol,quote_volume:x.quoteVolume,candles:await fetchCandles(x.symbol,timeframe)}}catch{return{symbol:x.symbol,quote_volume:x.quoteVolume,candles:[] as CandlePayload[]}}}));
    markets.push(...results);
   }

   setProgress("Running scanner filters on indicator-ready symbols");
 ...
 const stages=useMemo(()=>{
  ...
  const backendCandleDrops=new Set(p?.candles?.dropped_symbols??[]);
  const indicatorReady=topSymbols.filter(symbol=>!backendCandleDrops.has(symbol));
  const indicatorDiag:StageDiag|null=p?.candles?diag(topSymbols,indicatorReady):null;
  ...
  const scannerDiag:StageDiag|null=indicatorDiag&&latest?{
   passed:shortlistSymbols.length,
   dropped:Math.max(0,indicatorDiag.passed-shortlistSymbols.length),
   dropped_symbols:scannerDropped,
   passed_symbols:shortlistSymbols,
  }:null;
  ...
  return[
   ...
   {title:"Indicator Analysis",rule:"500 bars + ≥ 200 closed candles",diag:indicatorDiag},
 ...
 return <div className="pageWrap">
  <div className="pageHeader"><div><p className="eyebrow">Market Intelligence</p><h1>Scanner</h1><p className="muted">Full-universe funnel → top 30 unique assets → indicator analysis → final shortlist.</p></div><span className="modePill"><span />{scanning?"Scanner running":autoRun?"Auto scan active":"Scanner ready"}</span></div>
  ...
  <div className="statGrid">
   ...
   <div className="statCard"><span>Top Universe</span><strong>{frontPipeline?.top30.passed??0}</strong><small className="neutral">max 30 unique assets</small></div>
  ...
  <section className="panel" style={{padding:18,marginBottom:14}}>
   <div className="panelHead" style={{alignItems:"flex-start"}}>
    <div><p className="eyebrow">Scanner Pipeline</p><h2>Full Funnel Trace</h2><p className="muted" style={{marginTop:6}}>{progress}</p></div>
   ...
   <div style={pipelineWrap}>{stages.map((s,i)=><button key={s.title} onClick={()=>setSelectedStage(s.title)} style={{...pipelineCard,...(s.title===selectedStage?pipelineSelected:{})}}>
    <span className="eyebrow">Step {i+1}</span>
    <strong style={{display:"block",margin:"7px 0 5px",fontSize:12}}>{s.title}</strong>
    <span className="muted" style={{fontSize:9}}>{s.rule}</span>
   ...
   <div style={tracePanel}>
    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><strong>{activeStage.title} diagnostics</strong><div className="muted" style={{fontSize:10,marginTop:4}}>{activeStage.rule}</div></div><span className="periodTag">Pass {activeStage.diag?.passed??0} · Drop {activeStage.diag?.dropped??0}</span></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:12}}><div><div className="positive" style={{fontSize:10,fontWeight:700,marginBottom:7}}>PASSED SYMBOLS</div><div style={symbolBox}>{passedSymbols.length?passedSymbols.join(" · "):"None"}</div></div><div><div className="negative" style={{fontSize:10,fontWeight:700,marginBottom:7}}>DROPPED / STUCK HERE</div><div style={symbolBox}>{dropped.length?dropped.join(" · "):"None"}</div></div></div>
   </div>
  ...
  <section className="panel" style={{overflow:"auto"}}>
   ...
   <div style={{minWidth:1250}}><div style={{...rowStyle,color:"#69768a",fontSize:9,textTransform:"uppercase"}}><span>Symbol</span><span>TF</span><span>Side</span><span>Score</span><span>L/S</span><span>24h Quote Vol</span><span>Price</span><span>RSI</span><span>Vol Ratio</span><span>Reasons</span></div>
   {!latest&&<div style={{padding:28,color:"#78859a",fontSize:12}}>Run Scanner to populate the latest result.</div>}
   {latest?.candidates.map(r=><div key={`${r.symbol}-${r.side}`} style={rowStyle}><strong>{r.symbol}</strong><span>{r.timeframe}</span><strong className={r.side==="LONG"?"positive":"negative"}>{r.side}</strong><strong>{r.score}</strong><span>{r.long_score}/{r.short_score}</span><span>${fmt(r.quote_volume,0)}</span><span>{fmt(r.last_price,6)}</span><span>{fmt(r.rsi_14,2)}</span><span>{fmt(r.volume_ratio,2)}x</span><span title={r.reasons.join(" · ")}>{r.reasons.join(" · ")||"—"}</span></div>)}</div>
  </section>
 </div>
}
