...
export default function EngineWorkingLogPage(){
 ...
 return <div className="pageWrap">
  <div className="pageHeader"><div><p className="eyebrow">Engine Observability</p><h1>Engine Working Log</h1><p className="muted">Compact dropdown logs for each engine. Scanner now follows 1H trend → 15m setup → 5m entry.</p></div><span className="modePill"><span />{worker?.running?"Scanner worker active":"Scanner worker inactive"}</span></div>
  ...
  <details className="panel" style={detailsStyle}>
   <summary style={summaryStyle}><div><p className="eyebrow">1. Engine</p><strong>Indicator Engine</strong><div className="muted" style={{fontSize:10,marginTop:4}}>{indicatorLogs.length} manual log rows · USD-M futures candles</div></div><span className="periodTag">Open log ▾</span></summary>
   <div style={bodyStyle}>
  ...
  <details className="panel" open style={detailsStyle}>
   ...
   <div style={bodyStyle}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:10,marginBottom:12}}>
     ...
     <div style={miniCard}><span>Runs</span><strong>{worker?.run_count??0}</strong></div>
     <div style={miniCard}><span>Scan Pool</span><strong>{worker?.last_scan_pool??latest?.pipeline?.scan_pool?.passed??0}</strong></div>
...
const scannerRow={display:"grid",gridTemplateColumns:"180px 70px 80px 70px 85px 80px 65px 65px 65px 70px",gap:10,padding:"10px",borderBottom:"1px solid #171f2a",fontSize:10,alignItems:"center"} as const;
